...
int		temp_array(t_stacks *s, int len);
...
void	quick_sort2(t_stacks *s, int len)
{
	...
	median = (len / 2); // si chiffre impair on arrondi a la hausse.
	count = 0;
	while (dlst_len(s->stack_b->head) < median) // CEci ne fonctionne pas
	{
		if (s->stack_a->head->pos <= median)
		{
			push_b(s->stack_a, s->stack_b);
	...
	//if (dlst_len(s->stack_b->head) <= 13) // Si le nombre d'element qui sont pousse a la stack_b est plus petit que 13, un selection sort se fait. 
